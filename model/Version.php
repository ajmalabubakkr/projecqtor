...
class Version extends SqlElement {
  ...
  public function drawVersionsList($critArray,$withProjects=false) {
    ...
    foreach ($versList as $vers) {
      ...
      //$st=new Status($vers->idStatus);
      if ($vers->idStatus) {
        $result.= colorNameFormatter(SqlList::getNameFromId('Status', $vers->idStatus)."#split#".SqlList::getFieldFromId('Status', $vers->idStatus,'color'));
      }
      $result.= '</td></tr>';
    }
    return $result;
...
//Initial function to build a tree from a flat activity array
  ...
  protected function isRed($version,$way) {
    ...
    if (pq_strtotime($this->plannedDeliveryDate) < time()) {
      return true;
    }
    ...
    if ($way == 'composition') {
      return pq_strtotime($this->plannedDeliveryDate) > pq_strtotime($version->plannedDeliveryDate);
    }
    if ($way == 'structure') {
      return pq_strtotime($this->plannedDeliveryDate) < pq_strtotime($version->plannedDeliveryDate);
    }
  ...
  protected function statusVersionPlanning() {
    ...
    elseif (!$this->isStarted and !empty($this->plannedStartDate) and pq_strtotime($this->plannedStartDate) - time() < 0) {
    ...
    elseif ($this->isStarted and (empty($deliveryDate) or (!empty($this->plannedDeliveryDate) and pq_strtotime($this->plannedDeliveryDate) - time() > 0) or (!empty($this->plannedDeliveryDate) and strtotime($this->plannedEisDate) - time() > 0))) {
      $status = i18n('statusGanttInProgress');
    }
    elseif (!$this->isDelivered and $this->isEis and !empty($this->plannedDeliveryDate) and pq_strtotime($this->plannedDeliveryDate) - time() < 0) {

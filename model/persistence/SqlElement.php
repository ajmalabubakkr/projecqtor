...
public function getMailDetailFromTemplate($templateToReplace, $lastChangeDate=null, $isTitle=false) {
  ...
  public function recalculateCheckboxes($force = false) {
    ...
    if (self::$_skipAllControls) return;
    $status = new Status ( $this->idStatus );
    $class=get_class($this);
  ...
  public function buildSelectClause($included = false, $hidden = array(), $dep = array(), $parent = null) {
    ...
    foreach ( $this as $col => $val ) {
      ...
      and ( ($included and ($col == 'id' or $threeCars == 'ref' or $threeCars == 'top' or $col == 'idle') )
             or ($firstCar == '_') or (pq_strpos ( $this->getFieldAttributes ( $col ), 'hidden' ) !== false and pq_strpos ( $this->getFieldAttributes ( $col ), 'forceExport' ) === false and $col!='id') or ($col == 'password') or (isset ( $hidden [$col] )) or (pq_strpos ( $this->getFieldAttributes ( $col ), 'noExport' ) !== false) or (pq_strpos ( $this->getFieldAttributes ( $col ), 'calculated' ) !== false))) 
...
// MTY - LEAVE SYSTEM    
    ...
    if ($newProfile != '*') {
      ...
      $pe = $class.'PlanningElement';
      if(property_exists($class, $pe) and !$this->$pe->isManualProgress or $changeManualProgress and ($changeManualProgress->rightAccess == 2 or ! $changeManualProgress->id )){
        $listOther[]='progress';
      }
    }

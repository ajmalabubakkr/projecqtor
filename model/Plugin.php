...
class Plugin extends SqlElement {
    ...
    public function load($file) {
      ...
      $this->name=pq_str_replace('.zip','',$file['name']);
      $pos=pq_strpos(pq_strtolower($this->name),'_v');
      if ($pos) $this->name=pq_substr($this->name,0,$pos);
      ...
      foreach($value as $ind=>$prop) {
        ...
        if ($prop['tag']=='PROPERTY') {
          $name='plugin'.pq_ucfirst($prop['attributes']['NAME']);
        ...
        if (isset($pluginCompatibility) and !$testCompatibility) {
          ...
          global $version;
          if (version_compare(pq_ltrim($version,'V'),pq_ltrim($pluginCompatibility,'V'),"<")) {
      ...
      foreach ($langFileList as $testFile) {
        if (file_exists ( $testFile )) {
          ...
          while ( $line = fgets ( $file ) ) {
            ...
            if (isset ( $split [1] )) {
              $var = pq_trim ( $split [0], " \t" ); // Remove tabs and spaces
    ...
    public static function checkPluginCompatibility($version) {
      ...
      foreach (self::$pluginRequiredVersion as $versTest=>$plugins) {
        if (Tool::afterVersion($version,$versTest)) {
          foreach ($plugins as $namePlg=>$versPlgRequired) {
            ...
            if ($plg and $plg->id and Tool::beforeVersion($plg->pluginVersion,$versPlgRequired)) {
              $plg->idle=1;
              $plg->save();
        ...
        if (Tool::afterVersion($version,$versTest)) {
          foreach ($plugins as $compPlgName=>$compPlgVersion) {
            if ($compPlgName==$plgName) {
              if (Tool::beforeVersion($plgVersion, $compPlgVersion)) {
    ...
    public static function checkCustomDefinition($paramClass=null) {
        ...
        $list[]=$paramClass;
          ...
          while ( ($file = readdir($handle)) !== false) {
            if ($file == '.' || $file == '..' || $file=='index.php'   // exclude ., .. and index.php
                ...
                || pq_substr($file,0,1)=='Plg'                           // exclude custom Plugin files
                || pq_substr($file,0,1)=='_') {                          // exclude the _securityCheck.php
              continue;
            }
            $class=pathinfo($file,PATHINFO_FILENAME);
            if (!SqlElement::class_exists($class.'Main')) continue;
            $list[]=$class;
          }
          closedir($handle);
      }
      ...
      foreach ($list as $class) {
        ...
        $last=false;
        foreach ($obj->getFieldsList() as $fld) {
          if ($fld=='_Note' or $fld=='_Link' or $fld=='_Attachment' or $fld=='_sec_Link' or $fld=='_nbColMax') {
            $last=true;
          } else if ($fld=='_sortCriteriaForList' or $fld=='_calculateForColumn' or $fld=='_OtherClient' ) {
             continue; // specific field
          } else if ($last and ! $obj->isAttributeSetToField($fld,'hidden') and $fld!='_sec_restrictTypes' and $fld!='_spe_restrictTypes') { // not a specific field, after $last (notes, attachment, link)
            $errorClass[]=$class;
            errorLog("Customization issue for $class | $fld");
            break;
          }
        }
      }
      if (count($errorClass)==0) {
        ...
        if (pq_substr($msg,0,1)=='[') $msg="Some screen customizations are not consistent any more with current version";
        echo "$msg :<br/>";
        traceLog("$msg :");
        $allFixed=true;
        ...
        if (!$allFixed) {
          ...
          if (pq_substr($msg,0,1)=='[') $msg="Please access each screen on customization plugin to fix the issue (Plugin V6.1 or over is required)";
          echo $msg;
          traceLog($msg);
        } else {
          $msg=i18n("screenCustomizationErrorFixed");
          if (pq_substr($msg,0,1)=='[') $msg="Screen customizations have been fixed automatically ;)";
          echo $msg;
          if ($paramClass) {
          ...
          if ($plgCusto and $plgCusto->id and Tool::afterVersion($plgCusto->pluginVersion,'6.1')) {

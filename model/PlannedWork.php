...
// ================================================================================================================================
  ...
  public static function plan($projectIdArray, $startDate,$withCriticalPath=true,$infinitecapacity=false,$simulation=false,$criticalLike=false,$planRefType=null, $planRefId=null) {
...
//       }
    ...
    if ($planRefType and $planRefId and $pmCode=='DDUR' and isset($pe)) {
    ...
    if ($simulation or $criticalLike) CriticalResourceScenarioPool::getScenarioPoolInfo($listPoolExtraCapacity);
    $listPlan=self::sortPlanningElements($fullListPlan, $listProjectsPriority,$listProjectsType,$listProjectsDelay);
    $resources=array();
    $a=new Assignment();
    $topList=array();
    $reserved=array();
...
//       }
      ...
      if ($profile=="REGUL" or $profile=="FULL" 
       or $profile=="HALF" or $profile=="QUART") { // Regular planning
        ...
        $step=1;
      } else if ($profile=="FDUR" or $profile=="DDUR") { // Fixed duration
      ...
      foreach ($precList as $precId=>$precValArray) { // $precValArray = array(dependency delay,dependency type)
        ...
        if ($precTyp=='E-E' and ($profile=="FDUR" or $profile=="DDUR")) {
          $startPlan=addWorkDaysToDate($precEnd, $plan->validatedDuration *(-1) + 1 + $precVal,$plan->idProject);
      ...
      if ($plan->refType=='Milestone') {
        ...
        if ($profile=="FDUR" or $profile=="DDUR") {
          if (! $plan->realStartDate) {
            if ($plan->elementary) {
        ...
        if ($plan->indivisibility==1 and ($profile=='RECW' or $profile=='FDUR' or $profile=="DDUR" or $profile=="REGUL" or $profile=="FULL" or $profile=="HALF" or $profile=="QUART")) {
...
//          $idxAss=0;
          ...
          foreach ($listAss as $keyAss=>$ass) {
            ...
            $left=$ass->leftWork;
            $regul=false;
            if ($profile=="REGUL" or $profile=="FULL" or $profile=="HALF" or $profile=="QUART" or $profile=="FDUR" or $profile=="DDUR") {
              $endToTake=$endPlan;
            	...
            	for($tmpDate=$currentDate; $tmpDate<=$endPlan;$tmpDate=addDaysToDate($tmpDate, 1)) {
            		...
            		if ($infinitecapacity) {
            		  ...
            		  if (isset($ress['real-'.$tmpDate])) {
            		    $tempCapacity-=$ress['real-'.$tmpDate];
            		  }
...
//              self::storePlannedWorkLeveled($currentDate, $ass, $plan, $arrayPlannedWork, $changedAss, $left);              
              ...
              if ($regul and $infinitecapacity and isset($ress['real-'.$currentDate]) and $ress['real-'.$currentDate]>=$r->getCapacityPeriod($currentDate)) {
              ...
              $surbookedWork=0;
              $capacityNormal=null;
              if ($withProjectRepartition and isset($reserved['W']) and ! ($forceFDUR and ($profile=='FDUR' or $profile=="DDUR" or $profile=='REGUL' or $profile=='FULL' or $profile=='HALF' or $profile=='QUART'))) {
              ...
              if ($ress['variableCapacity'] or $infinitecapacity) {
                ...
                if ($forceFDUR and ($profile=='FDUR' or $profile=="DDUR" or $profile=='REGUL' or $profile=='FULL' or $profile=='HALF' or $profile=='QUART')) {
                  $capacityRate=$regul;
                  if ($regulNotPlanned) {
                    $max=$r->getCapacityPeriod($currentDate);
                    if ($capacityRate<$max) {
...
//               } else 
              ...
              if (isOpenDay($currentDate, $r->idCalendarDefinition)) {            
                ...
                if ( ! ($planned < $capacity or $profile=='RECW') )  {
                  ...
                  if ($withProjectRepartition and $profile!='RECW' and ! ($forceFDUR and ($profile=='FDUR' or $profile=="DDUR" or $profile=='REGUL' or $profile=='FULL' or $profile=='HALF' or $profile=='QUART'))) {
                    foreach ($listTopProjects as $idProject) {
                  ...
                  if ($infinitecapacity and isset($ress['real-'.$currentDate])) {
                    $expected=$value;
                    $value-=$ress['real-'.$currentDate];
                    $surbookedWork-=$ress['real-'.$currentDate];
                    if ($value<0) $value=0;
...
//                         }
                        ...
                        and $resources[$idRT]['periods'][$period]['idResource'][$ass->idResource]>0) {
                          ...
                          if ($infinitecapacity) {
                            ...
                            if (isset($listPoolExtraCapacity[$idRT])){
                              $capacityNormal+=$listPoolExtraCapacity[$idRT];
                            }
      ...
      // PBER - #8074 - START
      if ($plan->plannedStartDate and $plan->realStartDate) {
        $plan->plannedStartDate=($firstPlannedWork<$plan->realStartDate)?$firstPlannedWork:$plan->realStartDate;
      }
    ...
    foreach ($fullListPlan as $pe) {
      ...
      if ($pe->validatedEndDate) $pe->inheritedEndDate=null;
...
// Functions for PLAN
  ...
  private static function sortPlanningElements($list,$listProjectsPriority, $listProjectsType, $listProjectsDelay) {
    ...
    foreach ($list as $id=>$elt) {
      ...
      if ( $mainPriority=="endDate") $expectedEndDate=$elt->getExpectedEndDate($list,$listProjectsDelay[$elt->idProject]??null);
      ...
      $pm=$pmList[$elt->idPlanningMode];
    	if ($elt->elementary==0) {
    	  ...
    	  $crit='3'.pq_str_replace('-','',(($elt->validatedStartDate)?$elt->validatedStartDate:'9999-99-99'));
    	} else if ($pm=='FDUR' or $pm=="DDUR") { // FDUR  
    	  $crit='4'.$str01;
    	} else if ($pm=='RECW') { // RECW
    	  $crit='6'.$str01; // Lower priority (availability will be reserved)
    	} else if ($pm=='ALAP' and $expectedEndDate and $mainPriority=='endDate') {
    		$crit='5'.'.'.pq_str_replace('-','',(($expectedEndDate)?$expectedEndDate:'9999-99-99'));
      ...
      if ($mainPriority=='endDate' and $expectedEndDate) {
        $crit.='.'.pq_str_replace('-','',$expectedEndDate);
  ...
  private static function storePlannedWork(
    ...
    if (!$support) {
      ...
      if ($surbookedWork>0 and $plannedReserved>0 and $infinitecapacity and $profile!='DDUR' ) {

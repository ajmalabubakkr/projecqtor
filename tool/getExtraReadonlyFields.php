...
if($objectClass == 'Activity'){
  if(isset($_REQUEST['objectIdRow']) and isset($_REQUEST['editRowMode'])){
    $objectId=$_REQUEST['objectIdRow'];
  }else{
    $objectId = RequestHandler::getId('objectId');
  }
  $obj=new $objectClass($objectId);
  $progressMode = new ProgressMode($obj->ActivityPlanningElement->idProgressMode);
  if($progressMode->name != 'manual'){
    $result[] = 'editInputUnitProgress';
  }
}

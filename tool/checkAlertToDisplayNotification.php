...
foreach($lst as $alert) {
	if ($alert->alertDateTime<=$date) {
	  $title = $alert->title;
	  $remove = array("<td>","</td>");
	  $myMessage = $alert->message;
	  $finalMessage = pq_str_replace($remove, "", $myMessage);
	  $body = new Html2Text($finalMessage);
	  $message=$body->getText();
	  $type = $alert->alertType;
	  $idNotification = $alert->id;
	  $notification_array="#!#!#!#!#!#$title#!#!#!#!#!#$message#!#!#!#!#!#$type#!#!#!#!#!#$idNotification";
